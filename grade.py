...
'''
gt = gt.to_numpy() # list of [label, video_id]
pr = pr.to_numpy() # list of [label, video_id]
assert (len(gt) == len(pr)), "Length of answer and prediction is not matched."

pr_dict = {}
for label, video_id in pr:
    assert (video_id not in pr_dict), "Duplicated key exist in prediction, {}".format(video_id)
    pr_dict[video_id] = label
...
'''
    ...
    params
        y_true: list [label, video_id], ground truth label
        y_pred: dict {video_id: label}, predicted label
    return
        int, 1: if same, 0 if not

'''

def accuracy(y_true, y_pred):
    acc = []
    for gt_label, video_id in y_true:
        assert (video_id in y_pred), "Key {} is not in prediction.".format(video_id)
        acc.append(int(gt_label == y_pred[video_id]))
